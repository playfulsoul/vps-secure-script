...
assert_contains "$actual" $'security.firewall\tHost firewall management\t2.0.0-beta.2\tsecurity\tbuiltin' \
...
assert_contains "$actual" $'security.certificate\tNode certificate lifecycle\t2.0.0-beta.1\tsecurity\tbuiltin' \
    "module registry discovers the certificate lifecycle module"
