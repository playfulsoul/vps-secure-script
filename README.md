...
当前版本为 `2.0.0-dev`，已完成本地自动化测试，以及 Debian 12 和 Ubuntu 22.04 真实 VPS 验收，包括默认、双端口和仅自定义 SSH 端口，复杂既有 UFW 规则，Fail2Ban 安装与状态恢复、重启持久性、验证和回滚。其他目标系统和剩余高风险场景仍待验证，请先在有控制台和快照的临时机器上测试，不要直接部署到生产服务器。
...
首批目标为 Debian 11/12/13 和 Ubuntu 22.04/24.04，要求 systemd 与 APT。兼容性边界见 [COMPATIBILITY.md](COMPATIBILITY.md)，真实 Debian 验收步骤见 [docs/DEBIAN_TEST_PLAN.md](docs/DEBIAN_TEST_PLAN.md)，已完成的证据见 [Debian 12 验收记录](docs/DEBIAN_12_VALIDATION.md)和 [Ubuntu 22.04 验收记录](docs/UBUNTU_22_04_VALIDATION.md)。
