...
当前版本为 `2.0.0-dev`，已完成本地自动化测试和一轮 Debian 12 真实 VPS 验收，包括默认、双端口和仅自定义 SSH 端口，UFW 启用/停用状态恢复，Fail2Ban、重启持久性、验证与回滚。其他目标系统和剩余高风险场景仍待验证，请先在有控制台和快照的临时机器上测试，不要直接部署到生产服务器。
