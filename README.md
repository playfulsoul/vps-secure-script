...
2.0.0 与 rc.1 的功能代码相同，是完成 Beta、RC、公开发布和真实 VPS 升级恢复验证后的首个稳定版本。预发布用户检查更新后可以确认升级；正式版本默认只接收后续稳定更新。1.x 迁移助手固定下载并校验已经完成发布后验证的 2.0.0 正式归档。
...
单独导入公钥时**不会自动关闭密码登录**。这是有意的安全设计：密钥路径、客户端选择或云服务商配置稍有差异，立即关闭密码可能把用户锁在服务器外。

需要进一步强化时，请使用菜单中的“分阶段登录安全设置”。它会创建或复用普通 sudo 用户，导入 GitHub 公钥，并要求两次新窗口实际验证：首次验证后才能关闭密码登录，关闭后再次验证，才能限制 root。完整流程见 [分阶段登录安全设置](docs/LOGIN_HARDENING.md)。
...
| 分类 | 普通用户功能 |
|---|---|
| 安全防护 | SSH 端口状态、GitHub 公钥导入、分阶段登录强化、UFW、Fail2Ban |
