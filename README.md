...
`vps report` 会在平台状态目录的 `reports` 子目录生成一份权限为 `600` 的中文纯文本报告。
生成前会确认状态目录及报告目录属于当前管理员，且组用户和其他用户不可写；不符合时
直接停止，不会自动修改既有目录的权限。
它只读取受控的平台、系统、模块登记和事务存在性摘要，不执行安装、更新、修复、重启、
上传或自动提交。默认报告不包含完整 IP、主机名、用户名、域名、真实端口、密钥正文、
节点链接、认证参数、Fail2Ban 封禁地址、认证日志原文、环境变量、命令历史或配置全文。
终端只显示保存位置和脱敏提示；分享前仍应由用户自行打开检查。可用
`--output safe-name.txt` 指定同一受控目录内的新文件名，不能传入路径或覆盖已有文件。
