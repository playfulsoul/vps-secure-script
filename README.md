...
当前开发版本为 `2.0.0-beta.10`。它保留了 1.x 简单直观的彩色分区菜单，同时使用 2.x 模块化安全内核：执行前说明变化、保留当前 SSH 端口、执行后自动验证，并为关键操作保存回滚点。
...
curl -fLO https://github.com/playfulsoul/vps-secure-script/releases/download/v2.0.0-beta.10/vps-secure-platform-2.0.0-beta.10.tar.gz &&
curl -fLO https://github.com/playfulsoul/vps-secure-script/releases/download/v2.0.0-beta.10/vps-secure-platform-2.0.0-beta.10.tar.gz.sha256 &&
sha256sum -c vps-secure-platform-2.0.0-beta.10.tar.gz.sha256 &&
tar --no-same-owner --no-same-permissions -xzf vps-secure-platform-2.0.0-beta.10.tar.gz &&
./install.sh &&
vps
```

普通 sudo 用户应把最后两条命令改为 `sudo ./install.sh` 和 `sudo vps`。
校验成功时会显示 `vps-secure-platform-2.0.0-beta.10.tar.gz: OK`。
...
- 验证返回内容确实是 SSH 公钥；
- 显示密钥指纹；
- 保留已有授权的选项、注释与空行；相同公钥已存在时不重复追加，也不解除它的既有限制；
...
公钥回滚只撤销最近一次导入中能够确认归属的新增行，不会用旧备份覆盖后来添加的密钥。已经修改或出现重复、无法唯一确认归属的行会保留；原先不存在的授权文件，只有回滚后无其他内容才会删除。重复执行已完成的回滚不会再次删除密钥。

若用户身份、目录或符号链接发生变化，或旧版本事务缺少新增行证据，程序会拒绝自动回滚。写入失败不会报告成功；写入后的提交记录失败会尝试恢复原授权内容，恢复也失败时会明确报错并保留证据。中断事务需要人工核对，不会自动覆盖或继续。请保持已有 SSH 会话，不要直接删除事务记录或锁目录来强行重试。权限加固（主目录不可被组/其他用户写入、`.ssh` 为 `700`、授权文件为 `600`）不会通过回滚恢复为较弱权限。

此保护会检查写入前的身份与文件内容，但不是对其他程序同时编辑同一文件的互斥锁；导入和回滚期间请勿同时手动编辑授权文件。
...
在菜单中选择 **8. 诊断报告与程序维护**，可以检查并安装更新。更新包必须通过 SHA-256 校验，安装器会保留上一版本备份。

安装与版本恢复使用同一个互斥锁，先检查候选版本能否运行，再切换目录和 `vps` 入口；失败时尝试恢复原版本与入口。每次操作使用独立备份，恢复旧版不会消耗原备份。此处仅恢复平台程序，不撤销模块曾经修改的系统配置。

如果提示“上次操作中断”或“恢复也未完成”，不要反复安装，也不要直接删除锁或备份。默认锁目录为 `/usr/lib/vps-secure.operation-lock`，其中 `transaction` 文件指向本次事务目录；事务目录的 `paths`、`phase`、`old-link` 分别记录相关路径、阶段和原入口。先确认没有仍在运行的安装/恢复进程，再依据这些记录检查当前目录和备份，必要时人工恢复原目录与入口并运行 `vps --version` 验证。无法确定时保留现场寻求协助。强制终止或断电不会保证自动恢复；后续操作会拒绝越过遗留锁。
...
需要反馈问题时，选择 **8. 诊断报告与程序维护 → 5. 生成脱敏诊断报告**。
完成后会显示保存位置和分享前检查提示，按回车键返回维护菜单；报告保存在服务器本地，不会自动上传。
该菜单与 `vps report` 使用相同入口，在平台状态目录的 `reports` 子目录生成权限为 `600` 的中文纯文本报告。
