...
fail2ban_tx_prepare() {
    fail2ban_tx_phase "$1" prepared || return 40
...
fail2ban_tx_finish() {
    ...
    if [[ "$tx_armed" == yes ]]; then
        ...
        fail2ban_restore_dir "$tx_transaction" || recovery=$?
        if (( recovery == 0 )) && fail2ban_tx_phase "$tx_transaction" compensated; then
            rm -f "$tx_state/pending_transaction" || rc=60
            printf '原操作未完成，已恢复模块负责的配置；事务证据已保留。\n' >&2
        else
            fail2ban_tx_phase "$tx_transaction" recovery_failed || true
...
fail2ban_tx_run() (
    ...
    if [[ "$action" == rollback ]]; then
        ...
        fail2ban_tx_phase "$transaction" rollback_pending || return 60
        fail2ban_restore_dir "$transaction" || rc=$?
        if (( rc != 0 )); then
            printf '回滚未完成，保留现状和事务证据，请人工核对: %s\n' "$transaction" >&2
            return "$rc"
        fi
        fail2ban_tx_phase "$transaction" rolled_back || return 60
        rm -f "$tx_state/pending_transaction" || return 60
        return 0
    fi
    ...
    "fail2ban_${action}_impl" "$@" || return $?
    if [[ "$tx_armed" == yes ]]; then
        fail2ban_tx_phase "$tx_transaction" committed || return 40
...
fail2ban_apply() { fail2ban_tx_run apply; }
fail2ban_rollback() { fail2ban_tx_run rollback; }
